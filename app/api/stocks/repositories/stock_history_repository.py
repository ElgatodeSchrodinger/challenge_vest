...
class StockHistoryRepository:
    ...
    def get_current_day_indicators(self, company_id):
        ...
        currency_symbol = (
            today_stock_histories[0].currency_symbol
            if today_stock_histories
            else ""
        )
        return {
            "lowest": currency_symbol
            + str(today_prices and min(today_prices) or 0),
            "highest": currency_symbol
            + str(today_prices and max(today_prices) or 0),
            "average": currency_symbol
            + str(
                today_prices
                and round(sum(today_prices) / len(today_prices), 2)
                or 0
            ),
        }
