...
def test_buy_stocks(client):
    ...
    response = client.post("/transfers", json.dumps(data))
...
def test_sell_stocks(client):
    ...
    bought_response = client.post("/transfers", json.dumps(shares_to_buy_data))

    assert bought_response.status_code == 200
    assert bought_response.json()["qty"] == 10
    assert bought_response.json()["transaction_type"] == "buy"

    sold_response = client.post("/transfers", json.dumps(shares_to_sell_data))
...
def test_holding_stock_information(client):
    ...
    response_share_1 = client.post(
        "/transfers", json.dumps(bought_share_data_1)
    )
    response_share_2 = client.post(
        "/transfers", json.dumps(bought_share_data_2)
    )
    ...
    mystocks_response = client.get("/mystocks")
